...
Window::WindowClass::WindowClass() noexcept : hInst(GetModuleHandle(nullptr))
{
	...
	wc.hIcon       = static_cast<HICON>(
        LoadImage(hInst, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR));
	wc.hIconSm = static_cast<HICON>(
		LoadImage(hInst, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR));
	wc.hCursor       = nullptr;
...
Window::Window(int width, int height, const wchar_t *name) : width(width), height(height)
{
	...
	hWnd = CreateWindowW(WindowClass::GetName(), name, WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU,
...
std::optional<int> Window::ProcessMessages()
{
	...
	{
		if (msg.message == WM_QUIT) // check for quit
			return msg.wParam; // wParam is the Exit code send to PostQuitMessage
...
LRESULT CALLBACK Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSKEYDOWN:                                          // include ALT key(VK_MENU)
		if (!(lParam & 0x40000000) || kbd.AutorepeatIsEnabled()) // 눌린 키가 직전 키와 중복이
																 // 아니거나 중복 허용일  때
